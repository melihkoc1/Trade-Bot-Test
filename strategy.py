...
import sim_manager
from news.news_manager import get_sentiment_score as _news_get_sentiment
import news_scraper  # Legacy fallback
...
def _module_trend(result):
    ...
    if trend_ma and price:
        if price > trend_ma:
            score += 1
            reasons.append(
                f"[Trend] Fiyat {trend_ma_label} uzerinde — yapisal yukselis"
            )
        elif price < trend_ma * 0.95:
            score -= 2
            reasons.append(
                f"[Trend] Fiyat {trend_ma_label}'un %5+ altinda — yapisal dusus"
            )
        else:
            score -= 1
            reasons.append(f"[Trend] Fiyat {trend_ma_label} altinda — zayif yapi")
    ...
    if sma9 and sma21 and price:
        if sma9 > sma21 and price > sma21:
            score += 1
            reasons.append("[Trend] SMA: Fiyat ve kisa ort. yukarda")
        elif sma9 < sma21 and price < sma21:
            score -= 1
            reasons.append("[Trend] SMA: Fiyat ve kisa ort. asagida")
    ...
    if adx is not None:
        if adx >= 25:
            if td == "Yukari":
                score += 1
                reasons.append(f"[Trend] ADX guclu yukari trend ({adx})")
            elif td == "Asagi":
                score -= 1
                reasons.append(f"[Trend] ADX guclu asagi trend ({adx})")
    ...
    if st == 1:
        score += 1
        reasons.append("[Trend] SuperTrend: Yukari")
    elif st == -1:
        score -= 1
        reasons.append("[Trend] SuperTrend: Asagi")

    # Haftalik trend konfirmasyonu
    wt = result.get("weekly_trend")
    if wt == "Yukari":
        score += 1
        reasons.append("[Trend] Haftalik trend: Yukari")
    elif wt == "Asagi":
        score -= 1
        reasons.append("[Trend] Haftalik trend: Asagi")
...
def _module_momentum(result):
    ...
    if rs is not None:
        if rs >= 5:
            score += 1
            reasons.append(f"[Momentum] RS: BIST100 uzerinde (+{rs:.1f}%)")
        elif rs <= -10:
            score -= 1
            reasons.append(f"[Momentum] RS: BIST100 gerisinde ({rs:.1f}%)")
    ...
    if macd is not None and macd_sig is not None:
        if macd > macd_sig and macd > 0:
            score += 1
            reasons.append("[Momentum] MACD: Pozitif ve yukselme")
        elif macd < macd_sig and macd < 0:
            score -= 1
            reasons.append("[Momentum] MACD: Negatif ve dusus")

    # EMA crossover
    ec = result.get("ema_cross", "")
    if ec in ("Golden Cross", "Yukari"):
        score += 1
        reasons.append(f"[Momentum] EMA: {ec}")
    ...
    if result.get("bull_div", 0):
        score += 1
        reasons.append("[Momentum] Boga divergence tespit edildi")
    if result.get("bear_div", 0):
        score -= 1
        reasons.append("[Momentum] Ayi divergence tespit edildi")
...
def _module_timing(result):
    ...
    if rsi is not None:
        if rsi < 30:
            score += 2
            reasons.append(f"[Timing] RSI asiri satimda — potansiyel dip ({rsi})")
        elif rsi < 55:
            score += 1
            reasons.append(f"[Timing] RSI iyi giris bolgesi ({rsi})")
        elif rsi > 70:
            score -= 2
            reasons.append(f"[Timing] RSI asiri alimda — tehlikeli giris ({rsi})")
        elif rsi > 60:
            score -= 1
            reasons.append(f"[Timing] RSI yukselmis bolgede ({rsi})")
    ...
    if bb_u and bb_l and price:
        if price <= bb_l * 1.01:
            score += 1
            reasons.append("[Timing] Bollinger alt bandi — ucuz bolge")
        elif price >= bb_u:
            score -= 1
            reasons.append("[Timing] Bollinger ust bandi asildi — pahali bolge")
    ...
    if vol is not None:
        if vol >= 1.0:
            score += 1
            reasons.append(f"[Timing] Hacim onayliyor ({vol}x)")
        elif vol < 0.6:
            score -= 1
            reasons.append(f"[Timing] Dusuk hacim — guvenilmez ({vol}x)")
...
def _module_external(result, is_backtest):
    ...
    if ns >= 15:
        score += 1
        reasons.append(f"[External] Haber: Olumlu ({ns})")
    elif ns <= -15:
        score -= 1
        reasons.append(f"[External] Haber: Olumsuz ({ns})")
    ...
    if not is_backtest:
        ...
        if mv >= 6:
            score += 1
            reasons.append(f"[External] Minervini: Guclu ({mv}/8)")

        # Sektor makro skoru
        ss = result.get("sector_score", 0)
        if ss >= 3:
            score += 1
            reasons.append(f"[External] Sektor: Pozitif makro ({ss})")
        elif ss <= -3:
            score -= 1
            reasons.append(f"[External] Sektor: Negatif makro ({ss})")
        ...
        if rl_sig == "AL" and rl_conf >= 0.6:
            score += 1
            reasons.append(f"[External] RL Ajan: AL (%{rl_conf * 100:.0f} guven)")
        elif rl_sig == "SAT" and rl_conf >= 0.6:
            score -= 1
            reasons.append(f"[External] RL Ajan: SAT (%{rl_conf * 100:.0f} guven)")
...
def analyze_single_stock(
    ...
    result["rsi"] = (
        round(last["RSI_14"], 2) if last["RSI_14"] == last["RSI_14"] else None
    )
    result["macd"] = round(last["MACD"], 4) if last["MACD"] == last["MACD"] else None
    result["macd_signal"] = (
        round(last["MACD_Signal"], 4)
        if last["MACD_Signal"] == last["MACD_Signal"]
        else None
    )
    result["sma_9"] = (
        round(last["SMA_9"], 2) if last["SMA_9"] == last["SMA_9"] else None
    )
    result["sma_21"] = (
        round(last["SMA_21"], 2) if last["SMA_21"] == last["SMA_21"] else None
    )
    result["sma_50"] = (
        round(last["SMA_50"], 2) if last["SMA_50"] == last["SMA_50"] else None
    )
    result["sma_200"] = (
        round(last["SMA_200"], 2)
        if "SMA_200" in df.columns and last["SMA_200"] == last["SMA_200"]
        else None
    )
    result["bb_upper"] = (
        round(last["BB_Upper"], 2) if last["BB_Upper"] == last["BB_Upper"] else None
    )
    result["bb_lower"] = (
        round(last["BB_Lower"], 2) if last["BB_Lower"] == last["BB_Lower"] else None
    )
    result["minervini_score"] = (
        last["MINERVINI_SCORE"]
        if "MINERVINI_SCORE" in df.columns
        and last["MINERVINI_SCORE"] == last["MINERVINI_SCORE"]
        else 0
    )
    ...
    # --- NEWS SENTIMENT (Faz 27 → Faz 33: Gelişmiş Haber Analizi) ---
    if not is_backtest:
        try:
            news_score, news_list = _news_get_sentiment(symbol)
        except Exception:
            # Yeni sistem başarısız olursa eski sisteme fallback
    ...
    # --- ELITE VISION (Faz 29) ---
    result["supertrend_dir"] = (
        last["SUPERTREND_DIR"] if "SUPERTREND_DIR" in df.columns else 0
    )
    result["z_score"] = last["Z_SCORE"] if "Z_SCORE" in df.columns else 0
    result["gmma_div"] = (
        last["GMMA_DIVERGENCE"] if "GMMA_DIVERGENCE" in df.columns else 0
    )
    result["gmma_spread"] = last["GMMA_SPREAD"] if "GMMA_SPREAD" in df.columns else 0
    result["rwb_highway"] = (
        last["RWB_HIGHWAY"] if "RWB_HIGHWAY" in df.columns else False
    )
    ...
    # ADX
    adx_val = (
        last["ADX"] if "ADX" in df.columns and last["ADX"] == last["ADX"] else None
    )
    plus_di = (
        last["PLUS_DI"]
        if "PLUS_DI" in df.columns and last["PLUS_DI"] == last["PLUS_DI"]
        else None
    )
    minus_di = (
        last["MINUS_DI"]
        if "MINUS_DI" in df.columns and last["MINUS_DI"] == last["MINUS_DI"]
        else None
    )
    ...
    atr_val = (
        last["ATR_14"]
        if "ATR_14" in df.columns and last["ATR_14"] == last["ATR_14"]
        else None
    )
    ...
    if not is_index and not is_backtest:
        try:
            ...
            if df_bist is not None and len(df_bist) >= 20:
                ...
                index_ret = (
                    df_bist["Close"].iloc[-1] / df_bist["Close"].iloc[-lookback] - 1
                ) * 100
    ...
    if not is_backtest and interval in ["1d", "15m", "1h"]:
        try:
            ...
            if df_weekly is not None and len(df_weekly) > 30:
                ...
                wk_plus = (
                    wk_last["PLUS_DI"]
                    if wk_last["PLUS_DI"] == wk_last["PLUS_DI"]
                    else 0
                )
                wk_minus = (
                    wk_last["MINUS_DI"]
                    if wk_last["MINUS_DI"] == wk_last["MINUS_DI"]
                    else 0
                )
                wk_sma50 = (
                    wk_last["SMA_50"]
                    if wk_last["SMA_50"] == wk_last["SMA_50"]
                    else None
                )
    ...
    ema9 = (
        last["EMA_9"]
        if "EMA_9" in df.columns and last["EMA_9"] == last["EMA_9"]
        else None
    )
    ema21 = (
        last["EMA_21"]
        if "EMA_21" in df.columns and last["EMA_21"] == last["EMA_21"]
        else None
    )
    prev_ema9 = (
        prev["EMA_9"]
        if "EMA_9" in df.columns and prev["EMA_9"] == prev["EMA_9"]
        else None
    )
    prev_ema21 = (
        prev["EMA_21"]
        if "EMA_21" in df.columns and prev["EMA_21"] == prev["EMA_21"]
        else None
    )
        ...
        try:
            ...
            if fundamental:
                result["pe_ratio"] = (
                    ...
                    if fundamental.get("pe_ratio", 0) > 0
                    else None
                )
                result["pb_ratio"] = (
                    fundamental.get("pb_ratio")
                    if fundamental.get("pb_ratio", 0) > 0
                    else None
                )
                ...
                result["debt_to_equity"] = (
                    ...
                    if fundamental.get("debt_to_equity", 0) > 0
                    else None
                )
                result["roe"] = (
                    fundamental.get("roe") if fundamental.get("roe", 0) != 0 else None
                )
                ...
                if w52_high > 0 and w52_low > 0 and result["price"]:
                    ...
                    if w52_range > 0:
                        result["week52_position"] = round(
                            (result["price"] - w52_low) / w52_range * 100, 1
                        )
        except:
            pass
    ...
    if vol_avg_20 is not None and vol_avg_20 > 0:
        ...
        if vol_ratio >= 1.5:
            result["volume_trend"] = (
                "Guclu Alis Hacmi" if price_change > 0 else "Guclu Satis Hacmi"
            )
        elif vol_ratio >= 1.0:
    ...
    if result.get("anomaly_status"):
        ...
        result["reasons"].append(
            f"Anomali tespit edildi — AL oylari baskilandi (Skor: {result['anomaly_score']})"
        )
    ...
    max_sum = MODULE_MAX * (
        mw["trend"] + mw["momentum"] + mw["timing"] + mw["external"]
    )
    ...
    if result["signal"] in ("AL", "GUCLU AL"):
        ...
        if not (rsi_ok and adx_ok and volume_ok and struct_ok):
            ...
            failed = []
            if not rsi_ok:
                failed.append(f"RSI asiri alim ({result['rsi']})")
            if not adx_ok:
                failed.append(f"ADX zayif ({result['adx']})")
            if not volume_ok:
                failed.append(f"Dusuk hacim ({result['volume_ratio']}x)")
            if not struct_ok:
                _lbl = "SMA200" if _sma200 else "SMA50"
                failed.append(
                    f"Yapisal dusus (Fiyat {_price:.2f} < {_lbl} {_trend_ma:.2f} * 0.95)"
                )
    ...
    if len(df) >= 20:
        ...
        if is_near_peak:
            if vol_ratio_20 > 2.5:
                ...
                v_risk = 65  # Uyarı
            elif vol_ratio_20 > 1.3:
                v_risk = 40  # Dikkat

        result["volume_peak_risk"] = v_risk
        if v_risk >= 65:
            result["reasons"].append(
                f"🔴 Zirve Riski: Hacim son 20 gün ortalamasının {vol_ratio_20:.1f} katına çıktı! (Blow-off Top Riski)"
            )
...
def scan_all_bist50(period="6mo", interval="1d", profile_name=None):
    ...
    for i, symbol in enumerate(BIST50_SYMBOLS):
        ...
        result = analyze_single_stock(
            symbol, period=period, interval=interval, profile_name=profile_name
        )
...
if __name__ == "__main__":
    ...
    for pname in ["Trend Avcisi", "Deger Yatirimcisi"]:
        ...
        result = analyze_single_stock(
            test_symbol, period=p["period"], interval=p["interval"], profile_name=pname
        )
